...
open class BootstrapTask @Inject constructor(@Input val type: String) : DefaultTask() {
    ...
    private fun getArtifacts(): Array<JsonBuilder> {
        ...
        artifacts.add(JsonBuilder(
            "name" to cjar.name,
            "path" to "${Project.link}${cjar.name}",
            "size" to cjar.length(),
            "hash" to sha
        ))
    ...
    fun bootstrap() {
        val time = measureTimeMillis {
            val json = JsonBuilder(
                "projectVersion" to Project.clientVersion,
                "minimumLauncherVersion" to Project.launcherVersion,
            ...
            File(bootstrapDir, "bootstrap.json").writeText(prettyJson)

            Keys.sha256(
                File("key-private.pem"),
                File("${project.buildDir}/bootstrap/bootstrap.json"),
                File("${project.buildDir}/bootstrap/bootstrap.json.sha256")
            )
            if(!Project.disableAWS) {
                val client: S3Client = S3Client.builder().region(Region.US_EAST_2).build()

                File("${project.buildDir}/bootstrap/").walk().forEach {
                    if (!it.isDirectory) {
                        writeFiles(
                            client,
                            it.path.substringAfter("build\\bootstrap\\"),
                            it
                        )
                    }
                }
        ...
        File("${project.buildDir}/bootstrap/").delete()
        println("Client Release in: $time ms")

    }

    private fun writeFiles(client : S3Client, location : String, file : File) {
        val finalLocation = location.replace("\\","/")

        println("Uploading File: $finalLocation")
        val request = PutObjectRequest.builder().bucket(Project.bucketName).key(finalLocation).build()

        client.putObject(request, RequestBody.fromFile(file))

        val waiter = client.waiter()
        val requestWait = HeadObjectRequest.builder().bucket(Project.bucketName).key(finalLocation).build()
